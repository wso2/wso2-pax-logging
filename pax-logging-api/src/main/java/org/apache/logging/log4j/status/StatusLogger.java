...
public class StatusLogger extends AbstractLogger {
    ...
    public void logMessage(
        ...
        try {
            final StatusData statusData = createStatusData(fqcn, level, marker, message, throwable);
            buffer(statusData);
    ...
    private void notifyListeners(final StatusData statusData) {
        ...
        // Pax Logging - always notify default listeners
        if (!foundListeners || fallbackListener != null) {
    ...
    private StatusData createStatusData(
        ...
        return new StatusData(caller, level, marker, message, throwable, null, config.instantFormatter, instant);
    }
